...
result: [pending] — still requires a human visual pass (light/dark contrast, spacing, no regressions). To make this pass faster and not require running anything locally, `tests/e2e/phase5-rag.spec.ts` now also captures reference screenshots on every CI run under `test-results/phase5-visual/` (gitignored, regenerated per run): `kb-empty-state.png`, `kb-article-view.png`, `ticket-draft-grounded.png`, `ticket-draft-ungrounded.png`. These are light-mode-only, single-viewport captures — they narrow what a human needs to check (dark mode + responsive breakpoints) but do not replace the full §9 checklist pass.
