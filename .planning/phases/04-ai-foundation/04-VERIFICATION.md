...
**Disposition (07-09.1, CLOSED):** `tests/integration/egress-isolation.test.ts` now proves this by denial, not observation — it boots the real production image on a Docker network declared `internal: true` (no route to the public internet, verified experimentally to have neither an outbound nor an inbound default route), exercises all five AI/email flows (provider Test Connection, ticket creation + auto-triage, KB embed + grounded draft generation, an Insight run, an outbound email send) end to end, and asserts every one succeeds with zero internet route. A logging-DNS-resolver sidecar turns any *attempted* external lookup into an observed, asserted fact (the observed-destination set is asserted to be a subset of `{postgres, stub LLM, stub SMTP}`, not merely logged), and a negative control deliberately forces one real external lookup and confirms the capture actually records it — without that control, a capture that records nothing would look identical whether egress is truly absent or the capture itself is broken. **This run found a real, previously-undetected egress**: the Prisma CLI pings `checkpoint.prisma.io` on every `migrate deploy`, including inside the deny-all network — fixed via `CHECKPOINT_DISABLE=1` in the real `docker-compose.yml` (see `07-SECURITY-PASS.md`'s Human verification items #1 for the full account, including why 07-09's own static-analysis sweep had refuted this exact class of finding). **Honest residual (matches this item's own "why human" caveat):** the test still runs against a local Ollama-protocol stub and a stub SMTP server, not a real OpenAI/Anthropic endpoint or a real local Ollama binary — it cannot prove a real vendor SDK never phones home to its OWN infrastructure beyond the configured API host (e.g., a genuinely undocumented analytics call inside `openai`/`@anthropic-ai/sdk`). That residual sliver — "does the real, live SDK do anything the runtime-egress test's stub couldn't reveal" — is the one part of this item that remains irreducibly a live, real-credential human check, and is re-logged as such below (not a release gate).
...
**Disposition (07-09.1, SPLIT):** The *objective* half of this item — does the pipeline mechanically run end to end, and can a hostile/wrong model output ever corrupt what's displayed or trigger unintended actions — is now covered without a live credential: `tests/e2e/honesty-invariants.spec.ts` (adversarial-stub) proves a deliberately dishonest model's narrative can never move the SQL-derived numbers shown to the user, and that a fabricated citation is never rendered, and `tests/integration/egress-isolation.test.ts` (above) proves the full auto-triage pipeline runs to completion against a stub provider with no live credential required. The *subjective* half — "does a REAL vendor model's triage/classification actually feel correct on genuine, unscripted ticket text" — cannot be judged by any stub, by construction (a stub's output is authored, not inferred). **Re-logged as a quality-backlog item, explicitly NOT a release gate:** a maintainer should, at their convenience, configure a real or local Ollama provider and eyeball one real triage + one real draft before publicizing AIDA as production-ready, but this is a "does it feel good" check, not a correctness gate the codebase depends on. **Owner:** maintainer (Afrizzal), no target date — pick up alongside 06-HUMAN-UAT.md item 3's identical subjective residual (real-LLM output quality), since both need the same live-credential session.
