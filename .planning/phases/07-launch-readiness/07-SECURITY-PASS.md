...
- The three human-verification items carried forward since Phases 4-6 are **RESOLVED as of 07-09.1** (2026-08-01): each was replaced with an automated test wherever its content was objectively checkable, and the irreducibly subjective residual was re-logged as non-blocking feedback with a reason and an owner, not left open. See Human verification items below, `04-VERIFICATION.md`/`05-HUMAN-UAT.md`/`06-HUMAN-UAT.md` (updated in place), and `.planning/phases/07-launch-readiness/07-09.1-SUMMARY.md` for the full account.
...
**Status: RESOLVED (07-09.1), 2026-08-01.** Plan 07-09.1 (`.planning/phases/07-launch-readiness/07-09.1-PLAN.md`, `07-09.1-SUMMARY.md`) replaced these three items with automated tests wherever their content was objectively checkable, and re-logged only the irreducibly subjective residual. Each item's disposition is recorded below AND in its originating file (`04-VERIFICATION.md`, `05-HUMAN-UAT.md`, `06-HUMAN-UAT.md`), so no item stays open in two places. This section previously described a blocking human-verification checkpoint that 07-09.1's plan explicitly designed to replace; nothing below required an actual human sign-off session — every closure is backed by a test, and every residual is honestly re-logged as non-blocking, with a named reason and owner.

1. **Live network-egress capture** (04-VERIFICATION.md item 2) — **CLOSED**, with an honest residual. Sweep 5's static analysis enumerated every outbound call site in `src/` and found no hardcoded third-party host, but could not prove the absence of vendor-SDK-internal telemetry. `tests/integration/egress-isolation.test.ts` (07-09.1 Task 1) now proves this by denial: it boots the real production image on a Docker network with no route to the public internet, exercises all five AI/email flows to completion, and asserts the observed-destination set (via a logging-DNS-resolver sidecar) is a subset of `{postgres, stub LLM, stub SMTP}` — backed by a negative control that proves the capture mechanism can actually detect egress, not just fail to observe it. **This test found a real, previously-undetected egress**: the Prisma CLI pings `checkpoint.prisma.io` on every `migrate deploy`, inside the deny-all network. Fixed via `CHECKPOINT_DISABLE=1` in the real `docker-compose.yml:35` (and the egress-test compose file). **Noteworthy for this report's own methodology:** Sweep 5 (row F-14 in the Findings table above) had already surfaced the Prisma-checkpoint ping as a build/deploy-time issue and it was left as a "known issue, not in approved-fixes scope" — the static sweep correctly IDENTIFIED it but, being static analysis, had no way to prove it also fires at RUNTIME inside a supposedly network-isolated deployment; the runtime egress test caught the same issue from a different angle and it got fixed as a direct consequence. **Residual (matches the original item's own caveat, now narrower):** the test still uses a local Ollama-protocol stub and a stub SMTP server, not a real OpenAI/Anthropic endpoint or a real local Ollama binary — it cannot prove a live vendor SDK never calls its OWN infrastructure beyond the configured API host. That sliver is re-logged in `04-VERIFICATION.md` item 2 as a genuine, non-blocking human-only residual (no target date; pick up alongside item 2 below).
2. **Live-provider smoke test** (04-VERIFICATION.md item 3 + 06-HUMAN-UAT.md item 3) — **SPLIT, objective half CLOSED, subjective half re-logged as a non-blocking quality backlog item.** The objective claim — a hostile/wrong model output can never corrupt what's displayed to the user or trigger an unintended action — is now proven adversarially by `tests/e2e/honesty-invariants.spec.ts` (07-09.1 Task 2): (a) a deliberately dishonest Insight narrative ("0% breach rate, 100 responses") cannot move the SQL-derived numbers `sla-csat-card.tsx:23`'s LOCKED invariant declares must never come from `narrative.summary`; (b) a draft citing a chunk that was never retrieved renders no citation for it, proven against `generate-draft.ts`'s real `citationsResolved` filter, with the raw pre-filter model output confirmed to genuinely contain the fabricated id (not a vacuous pass); (c) the zero-retrieval no-hallucination gate was found to already be covered by `tests/e2e/phase5-rag.spec.ts`'s "Ungrounded draft" test and was not duplicated. The subjective claim — does a REAL vendor/local-Ollama model's triage classification, draft prose, cluster labels, and KB-gap matches actually look good on genuine ticket content — cannot be judged by any stub, by construction. **Re-logged as a quality-backlog item, explicitly NOT a release gate**, in both `04-VERIFICATION.md` item 3 and `06-HUMAN-UAT.md` item 3. **Owner:** maintainer (Afrizzal), no target date — one combined session with a real or local-Ollama credential covers both.
3. **DESIGN-SYSTEM §9 visual pass** (05-HUMAN-UAT.md item 3) — **CLOSED**, mechanical half; aesthetic half re-logged. 07-08's Task 3 sign-off already discharged 5 surfaces (inbox, ticket-detail, insights, KB list, AI settings) in both themes. This item named two surfaces 07-08 never captured — `/kb/new`, `/kb/[id]` — plus the ticket page's draft card specifically in its **in-flight** (generated, not yet inserted) state, distinct from `ticket-detail.png`'s static capture of an already-**resolved** Triage → Draft generated → Draft approved trail. `tests/e2e/a11y-contrast.spec.ts` (07-09.1 Task 3) now asserts zero axe-core `color-contrast` violations, in both themes, on exactly those surfaces (plus `/kb` itself, for completeness), and `tests/unit/design-tokens.test.ts` mechanically enforces the companion "no hardcoded color, no named Tailwind text size" rule across every component file. Writing and running these tests found and FIXED four real, isolated contrast bugs (a `Badge` silently inheriting `bg-primary` due to a missing `variant="outline"` in four components; the sidebar footer email's `/60`-opacity muted text) and two real token violations (the login/setup page titles' `text-2xl`, a login error message's `text-sm`) — see `07-09.1-SUMMARY.md`. **What remains open, re-logged, NOT a release gate:** (a) `--primary` (both themes) and `--success` (light theme) themselves fall short of 4.5:1 in several first-class usages (default buttons, text-on-dark-surface, one badge) — a genuine, systemic brand/semantic-color VALUE gap, not a narrow bug; left deliberately unfixed (not allowlisted) per this plan's own critical constraint, and logged with a reason and an owner (maintainer, no target date) in `deferred-items.md`'s "From 07-09.1" section. (b) Pure aesthetic judgement (spacing, "does this look good") — per 07-09.1's own D-3, this was never intended to be automatable or a release gate, and is re-logged as ungated feedback only.
