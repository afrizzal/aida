...
- [ ] 03-01-PLAN.md — Deps + Message email fields/deliveryStatus enum + EmailIngestFailure model + scopedDb allowlist (Wave 1)
- [ ] 03-02-PLAN.md — AES-256-GCM secret-box helper (TDD) + typed email-settings module over Setting store (Wave 1)
- [ ] 03-03-PLAN.md — Parsing primitives: sanitizeEmailHtml + extractEmailBody + thread-match + isAutoGenerated (Wave 2)
- [ ] 03-05-PLAN.md — Outbound SMTP send job + boss-client enqueue + deliveryStatus + Failed-to-send/Retry (Wave 2)
- [ ] 03-04-PLAN.md — Inbound ingest + IMAP poll job + poison guard + auto-reopen + worker wiring (Wave 3)
- [ ] 03-06-PLAN.md — Settings Email tab: toggle + IMAP/SMTP/from form + Test Connection + health line (Wave 4)
